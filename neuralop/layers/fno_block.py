...
class HyperConnection(nn.Module):
    ...
    def width_connection(self, h: torch.Tensor):
        ...
        static_beta = self.static_beta.view(1, self.rate, 1, *([1] * len(spatial_shape)))

        if self.dynamic:
            ctx = h.mean(dim=1)
            norm_ctx = self.layer_norm(ctx)

            beta = self.dynamic_beta_fn(norm_ctx)
            beta = torch.tanh(beta)
            beta = beta.unsqueeze(2)

            beta = static_beta + beta * self.dynamic_beta_scale
        else:
            beta = static_beta.expand(batch_size, -1, -1, *spatial_shape)

        return h_in, beta

    def depth_connection(self, h: torch.Tensor, h_out: torch.Tensor, beta: torch.Tensor):
        update = h_out.unsqueeze(1) * beta
        return h + update
